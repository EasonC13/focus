  ...
  <div class="container view_log_container" style="position: absolute">
    <div class="row row_" v-if="ready">
      ...
      <div class="col-sm-4 col-sm-4_">
        ...
        <ExportLogToCsv
          :storage_id="currentData"
          :showHint="false"
        ></ExportLogToCsv>
        <p>
          Full version includes emotional probability, screenshots, and eye
          movements. <br />Because the content is large, it needs to be analyzed
          by a program, or you can import it again to this website for analysis
        </p>
        <FileReader
  ...
  methods: {
    init() {
      ...
      all_data = all_data.filter((x) => {
        return (
          (JSON.parse(localStorage.getItem(x)) || { logs: [] }).logs.length > 0
        );
      });
      localStorage.setItem("ids", JSON.stringify(all_data));
      this.dataList = all_data;
      this.currentData =
        this.$route.query.data || all_data[all_data.length - 1];
      this.$router
        .replace({ query: { data: this.currentData } })
        .catch(() => {});
    ...
    initFrame() {
      ...
      for (let i_ in log.gaze_log) {
        ...
        this.gazer_points.push({
          x: Number.parseInt(
            (log.gaze_log[i_][0] / log.window_inner_size[0]) * this.img_width
          ),
          y: Number.parseInt(
            (log.gaze_log[i_][1] / log.window_inner_size[1]) * this.img_height
          ),
          value: value,
        });
