<template lang="">
  <div>
    <button @click="exportCSV" :class="this.class_text">{{ text }}</button>
    <input
      type="checkbox"
      id="jack"
      class="ml-3 mr-1"
      value="Export CSV which only can be read by program"
      v-model="checkedForProgramVer"
    />
    <label for="jack">Full Version</label>
  </div>
</template>
<script>
function sleep(time) {
